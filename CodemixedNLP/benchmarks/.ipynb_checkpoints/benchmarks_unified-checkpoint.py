        ...
        if selected_examples:
            ...
            for batch_id, batch_examples in enumerate(selected_examples_batch_iter):
                ...
                for target_label_field, target_label_vocab, task_attribute in \
                    ...
                    if task_attribute["name"] == "classification":
                        _targets = [target_label_vocab.token2idx[tgt] for tgt in batch_targets]
                        target_lens.extend([1 for tgt in batch_targets])
                    elif task_attribute["name"] == "seq_tagging":
                        _targets = [[target_label_vocab.token2idx[token] for token in tgt.split(" ")] for tgt in
                                    batch_targets]
                        target_lens.extend([len(tgt.split(" ")) for tgt in batch_targets])
                    targets.append(_targets)
                # run forward
                test_exs.extend(batch_sentences_trimmed)
                output_dict = model.predict(inputs=inputs, targets=targets)  # enhanced dict compared to forward
                # loss and accuracy
                batch_loss = output_dict["loss"].cpu().detach().numpy()
            ...
            if i == 0:
                opfile = open(os.path.join(ckpt_path, "predictions_dev.txt"), "w", encoding="utf-8")
            else:
                opfile = open(os.path.join(ckpt_path, "predictions_test.txt"), "w", encoding="utf-8")
            idx = 0
            for i, (x, l) in enumerate(zip(test_exs, target_lens)):
                y = ""
                z = ""
                for c in range(l):
                    y += target_label_vocab.idx2token[test_preds[idx]] + " "
                    z += target_label_vocab.idx2token[test_true[idx]] + " "
                    idx += 1
                print(x.strip() + "\t" + z.strip() + "\t" + y.strip(), file=opfile)
            opfile.close()
